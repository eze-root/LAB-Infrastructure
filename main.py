...
class RemoteServer:
    ...
    def download_latest_exporter(cls, os_arch):
        ...
        cls._download_node_exporter(os_arch)
        cls._download_nvidia_gpu_exporter(os_arch)
        logger.info("All exporters downloaded successfully.")

    @classmethod
    def _download_clash_exporter(cls, arch="linux-amd64"):
    ...
    @classmethod
    def _download_node_exporter(cls, arch="linux-amd64"):
    ...
    @classmethod
    def _download_nvidia_gpu_exporter(cls, arch="linux-amd64"):
        ...
        if res.returncode != 0:
            raise RuntimeError(f"Failed to install clash exporter: {res.stderr}")
        logger.info(
            f"Nvidia-GPU exporter installed successfully for {arch} architecture."
        )
        ...
        async with asyncssh.connect(
            ...
            for exporter in self.get_exporter_for_running():
                ...
                if result.exit_status == 0:
                    logger.info(
                        f"Exporter already running on remote server {self.name}. Skipping download."
                    )
                    continue
    ...
    async def get_remote_os(self):
        ...
        async with asyncssh.connect(
            ...
            result = await conn.run(
                "echo $(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)", check=True
            )
    ...
    async def check_and_start_exporter(self):
        async with asyncssh.connect(
            ...
            for ind, exporter in enumerate(self.get_exporter_for_running()):
                # 1. 检查是否已运行
                check_cmd = (
                    f'ps -eo args | grep "^/tmp/exporter/{exporter} " | grep -v grep'
                )
                result = await conn.run(check_cmd, check=False)
                if result.exit_status == 0:
                    logger.info(
                        f"{exporter} already running on remote server {self.name}. Skipping start."
                    )
                    continue
                ...
                logger.info(
                    f"{exporter} start command sent to remote server {self.name}"
                )
    ...
    async def start_single_tunnel(self, ind, exporter):
        while True:
            try:
                ...
                print(
                    f"[{self.name}] SSH tunnel for {exporter} exited: {e}. Reconnecting in 5s..."
                )
    ...
    async def run(self):
        ...
        while True:
            try:
                arch = await self.get_remote_os()
                logger.info(f"[{self.name}] remote arch={arch}, has_gpu={self.has_gpu}")
                self.download_latest_exporter(arch)
                
                await self.upload_exporter()
                await self.check_and_start_exporter()
                await self.start_ssh_tunnel()
            except Exception as e:
                logger.error(f"[{self.name}] run loop error: {e}. Retry in {backoff}s")
                await asyncio.sleep(backoff)
                backoff = min(backoff * 2, 60)  # 指数退避到 60s 上限
            backoff += 5
